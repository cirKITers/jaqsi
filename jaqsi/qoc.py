...
from jaqsi.gates import Gates, PulseInformation, PulseEnvelope
from jaqsi import operations as op
from jaqsi import gateset
from jaqsi.math import phase_difference, fidelity
...
def fidelity_cost_fn(
    ...
    pulse_scripts: Union[Script, List[Script]],
    target_scripts: Union[Script, List[Script]],
    n_samples: int,
) -> Tuple[float, float]:
    """
...
def unitary_cost_fn(
    pulse_params: jnp.ndarray,
    pulse_basis_scripts: List[Script],
    target_basis_scripts: List[Script],
    n_samples: int,
    n_qubits: int,
) -> Tuple[float, float]:
...
def joint_unitary_cost_fn(
    ...
    gate that shares the joint parameter vector ``pulse_params``::

        {
            ...
            "pulse_basis_scripts":  List[Script], # 2**n_qubits scripts
            "target_basis_scripts": List[Script],
        }
...
def profile_pulse_pipeline(
    ...
    the patch notes:
        ...
        from jaqsi.qoc import profile_pulse_pipeline
    ...
    with PulseInformation.preserve_state():
        ...
        pulse_script = Script(pulse_circuit, n_qubits=n_qubits)
        target_script = Script(target_circuit, n_qubits=n_qubits)
...
if __name__ == "__main__":
    ...
    # create logger
    log = logging.getLogger("jaqsi.qoc")
