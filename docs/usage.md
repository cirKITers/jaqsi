...
- `__init__.py` : the package entry point. Re-exports `Script` for circuit building, the `Hamiltonian` factory for time-evolution sources, the gate front-ends (`Gates`, `UnitaryGates`, `PulseGates`) and the quantum-info helpers, so that `import jaqsi` is enough for everyday use. Time evolution is invoked as a method on the Hamiltonian object (`hamiltonian.evolve(...)`); the `Evolution` engine is re-exported for solver configuration (`Evolution.set_solver_defaults`).
